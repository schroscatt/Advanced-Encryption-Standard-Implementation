...
public class AES {
	public static final int[] sbox = {
		...
		0x54, 0xbb, 0x16};
	public static final int[] invsbox = {0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3,
	...
	public static final int[][] Rijndaelmatrix={{2,3,1,1},{1,2,3,1},{1,1,2,3},{3,1,1,2}};
	public static final int[][] invMixColumnMatrix={{14,11,13,9},{9,14,11,13},{13,9,14,11},{11,13,9,14}};


public static final int [] multiplyBy9={0x00,0x09,0x12,0x1b,0x24,0x2d,0x36,0x3f,0x48,0x41,0x5a,0x53,0x6c,0x65,0x7e,0x77,
...
public static final int [] multiplyBy11 = {0x00,0x0b,0x16,0x1d,0x2c,0x27,0x3a,0x31,0x58,0x53,0x4e,0x45,0x74,0x7f,0x62,0x69,
...
public static final int [] multiplyBy13={0x00,0x0d,0x1a,0x17,0x34,0x39,0x2e,0x23,0x68,0x65,0x72,0x7f,0x5c,0x51,0x46,0x4b,
...
public static final int [] multiplyBy14={0x00,0x0e,0x1c,0x12,0x38,0x36,0x24,0x2a,0x70,0x7e,0x6c,0x62,0x48,0x46,0x54,0x5a,
...
public static final int [] rconst = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36};
	...
	private static int [][] mixColumns(int [][]state,boolean dec){
		...
		if(!dec){
			for(int rowRijndael=0;rowRijndael<4;rowRijndael++){
				for(int stateCol=0;stateCol<4;stateCol++){
					for(int cell=0;cell<4;cell++){
						if(Rijndaelmatrix[rowRijndael][cell]==1){
							temp[rowRijndael][stateCol]^=state[cell][stateCol];

						}else if(Rijndaelmatrix[rowRijndael][cell]==2){
							int shiftedNum=multiplyBy2(state[cell][stateCol]);
							temp[rowRijndael][stateCol]^=shiftedNum;
						}else{
							int shiftedNum=multiplyBy2(state[cell][stateCol]);
							temp[rowRijndael][stateCol]^=shiftedNum^state[cell][stateCol];
						}
					}
				}
			}
				...
				for(int stateCol=0;stateCol<4;stateCol++){
					for(int cell=0;cell<4;cell++){
						...
						if(invMixColumnMatrix[invMixRow][cell]==9){
							...
							temp[invMixRow][stateCol]^=multiplyBy11[state[cell][stateCol]];
						}else if(invMixColumnMatrix[invMixRow][cell]==13){
							temp[invMixRow][stateCol]^=multiplyBy13[state[cell][stateCol]];
						}else{
							temp[invMixRow][stateCol]^=multiplyBy14[state[cell][stateCol]];
						}
					}
				}
			}
